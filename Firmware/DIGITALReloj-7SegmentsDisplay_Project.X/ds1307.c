...
void Write_Byte_To_DS1307_RTC(unsigned char Address, unsigned char DataByte){
	...
	while(I2C1_Send(Device_Address_DS1307_EEPROM + 0) == 1)// Wait until device is free
	{	I2C1_Start();	}		
	I2C1_Send(Address);							// Write Address byte
	I2C1_Send(DataByte);							// Write data byte
...
unsigned char Read_Byte_From_DS1307_RTC(unsigned char Address)
{
	...
	I2C1_Send(Address);							// Write Address byte
	I2C1_ReStart();										// Restart i2c

	// Send i2c address of DS1307 RTC with read command	
	I2C1_Send(Device_Address_DS1307_EEPROM + 1);		

	Byte = I2C1_Read();								// Read byte from EEPROM

	I2C1_Send_NACK();									// Give NACK to stop reading
	I2C1_Stop();											// Stop i2c communication
...
void Write_Bytes_To_DS1307_RTC(unsigned char Address,unsigned char* pData,unsigned char NoOfBytes)
{
	...
	for(i=0;i<NoOfBytes;i++)							// Write NoOfBytes
		I2C1_Send(pData[i]);						// Write data byte
...
void Read_Bytes_From_DS1307_RTC(unsigned char Address, unsigned char* pData, unsigned int NoOfBytes)
{
	...
	pData[0] = I2C1_Read();							// Read First byte from EEPROM

	for(i=1;i<NoOfBytes;i++)							// Read NoOfBytes
	{		
		I2C1_Send_ACK();					// Give Ack to slave to start receiving next byte
		pData[i] = I2C1_Read();		// Read next byte from EEPROM
	}
